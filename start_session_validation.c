...
#include<stdbool.h>
#define MAX 255
int  start_session_validation(char *str,int uid)
{
    int i=0,len=0,rv=0;
    len=strlen(str);
    str[len]='\0';
    ...
    regfree(&re);
 	if (status == 0) {
		  rv=start_length_checking(str,uid);
			if(rv==1)
		...
		start_validate_buffer(str,uid);         	
    
    
}    
int  start_validate_buffer(char *cBuffer,int uid){
...
int start_length_checking(char *cBuffer,int reg_uid){
	...
	if(cBuffer[0]!='\n'){
		...
		if((sid_category<13) && (sid_category>0))
			flag1=true;
		else
			printf("\n Incorrect! ,service sid_category should be maximum of 12 characters, re-enter again\n");
